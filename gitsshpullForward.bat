...
rem ssh -t -p 3182 sr-projekt-execs-2@h2899502.stratoserver.net "cd ~/mys-projekt-load-balancer-auto-scaler ; git reset --hard HEAD && git pull && ~/node_modules/pm2/bin/pm2 start ~/mys-projekt-load-balancer-auto-scaler/ecosystem.config.js --only "execs""
...
ssh -p 3154 sr-projekt-execs-1@h2899502.stratoserver.net "bash ~/mys-projekt-load-balancer-auto-scaler/gitsshpullLocal.sh" 